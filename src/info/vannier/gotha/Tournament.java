...
public class Tournament extends UnicastRemoteObject implements TournamentInterface, java.io.Serializable {
    ...
    private String remoteUUID ="defaultUUID";
    private String remoteRunningMode = "---";
    private String remoteFullVersionNumber = "x.yy";
    private boolean privateTournament = true;
...
/**
    ...
    /**
     * @param remoteUUID the remoteUUID to set
     */
    ...
    @Override
    public boolean remoteAccessAuthorized() throws RemoteException{
        if(!this.isPrivateTournament()) return true;
        
        if (Gotha.isAdmin()) return true;
        
        String strRemoteUUID = this.getRemoteUUID();
        if (strRemoteUUID.equals("00000000-0000-0000-0000-000000000000")) return true; // xml files generaed by old version will be visile
        String strLocalUUID = Gotha.getLocalUUID();
        if (getRemoteUUID().toString().equals(strLocalUUID.toString())) return true;
    ...
    @Override
    public int numberOfPlayersWithRankEqualTo(int rank) throws RemoteException {
        int nb = 0;
        for (Player p : hmPlayers.values()) {
            if (p.getRank() == rank) {
                nb++;
    ...
    @Override
    public int numberOfPlayersInBZHGroup(int numBZG, ArrayList<Player> alP) throws RemoteException {
        GeneralParameterSet gps = tournamentParameterSet.getGeneralParameterSet();
        int nb = 0;
        for (Player sp : alP) {
            if (sp.bzhGroup(gps) == numBZG) {
                nb++;
            }
        }
        return nb;
...
//        if (hdPS.isHdBasedOnMMS()) {    // Obsolete since V3.51.04
...
//        }
        switch(hdPS.getHdBase()){
            case HandicapParameterSet.HD_BASED_ON_MMS : 
                pseudoRank1 = sP1.getCritValue(PlacementParameterSet.PLA_CRIT_MMS, roundNumber - 1) / 2 + Gotha.MIN_RANK;
                pseudoRank2 = sP2.getCritValue(PlacementParameterSet.PLA_CRIT_MMS, roundNumber - 1) / 2 + Gotha.MIN_RANK;
                break;
            case HandicapParameterSet.HD_BASED_ON_BZS : 
                // pseudoRank1 = sP1.getCritValue(PlacementParameterSet.PLA_CRIT_BZS, roundNumber - 1) / 2 + Gotha.MIN_RANK;
                // pseudoRank2 = sP2.getCritValue(PlacementParameterSet.PLA_CRIT_BZS, roundNumber - 1) / 2 + Gotha.MIN_RANK;
                break;
    ...
    private void fillBaseScoringInfoIfNecessary() throws RemoteException {
            ...
            for (Game g : hmGames.values()) {
                ...
                switch (g.getResult()) {
                    ...
                    case Game.RESULT_WHITEWINS_BYDEF:
                        ...
                        wSP.setBZSX2(r, wSP.getBZSX2(r) + 2);
                        break;
                    case Game.RESULT_BLACKWINS:
                    case Game.RESULT_BLACKWINS_BYDEF:
                        ...
                        bSP.setBZSX2(r, bSP.getBZSX2(r) + 2);
                        break;
                    case Game.RESULT_EQUAL:
                    case Game.RESULT_EQUAL_BYDEF:
        ...
        for (int r = 0; r < numberOfRoundsToCompute; r++) {
            ...
            for (ScoredPlayer sp : hmScoredPlayers.values()) {
                if (r == 0) {
                    ...
                    sp.setBZSVirtualX2(r, 2 * sp.sbzs(gps));
                } else {
                    ...
                    sp.setBZSVirtualX2(r, sp.getBZSVirtualX2(r - 1));
                }
            }
            ...
            for (Game g : hmGames.values()) {
                ...
                switch (g.getResult()) {
                    ...
                    case Game.RESULT_WHITEWINS:
                        ...
                        wSP.setBZSVirtualX2(r, wSP.getBZSVirtualX2(r) + 2);
                        break;
                    case Game.RESULT_BLACKWINS:
                        ...
                        bSP.setBZSVirtualX2(r, bSP.getBZSVirtualX2(r) + 2);
                        break;
                    case Game.RESULT_EQUAL:
                        ...
                        wSP.setBZSVirtualX2(r, wSP.getBZSVirtualX2(r) + 1);
                        ...
                        bSP.setBZSVirtualX2(r, bSP.getBZSVirtualX2(r) + 1);
                        break;
                    case Game.RESULT_BOTHWIN:
                        wSP.setNBWVirtualX2(r, wSP.getNBWVirtualX2(r) + 2);
                        wSP.setCPSVirtualX2(r, wSP.getCPSVirtualX2(r) + 2);
        ...
        for (ScoredPlayer sp : hmScoredPlayers.values()) {
            ...
            for (int r = 0; r < numberOfRoundsToCompute; r++) {
                ...
                sp.setBZSVirtualX2(r, sp.getBZSVirtualX2(r) + nbVPX2);  
            } 
        }
...
//                        ostsX2[rr] = 2 * sp.smms(gps);
        ...
        // 4.2) SOSM, SOSMM1, SOSMM2, SODOSM, SOSB, SOSBM1, SOSBM2, SODOSB, SOSTS
        for (int r = 0; r < numberOfRoundsToCompute; r++) {
            for (ScoredPlayer sp : hmScoredPlayers.values()) {
                int[] osmX2  = new int[numberOfRoundsToCompute];
                int[] osbX2  = new int[numberOfRoundsToCompute];                
                int[] dosmX4 = new int[numberOfRoundsToCompute];    // Defeated opponents score
                int[] dosbX4 = new int[numberOfRoundsToCompute];    // Defeated opponents score
                int[] ostsX2 = new int[numberOfRoundsToCompute];
                for (int rr = 0; rr <= r; rr++) {
                    if (sp.getParticipation(rr) != ScoredPlayer.PAIRED) {
                        osmX2[rr] = 2 * sp.smms(gps);
                        osbX2[rr] = 2 * sp.sbzs(gps);
                        ostsX2[rr] = 2 * sp.smms(gps);
                    } else {
                        Game g = sp.getGame(rr);
                        ...
                        dosbX4[rr] = osbX2[rr] * getWX2(g, sp);
                ...
                sp.setSOSBX2(r, sosBX2);
                sp.setSDSMX4(r, sdsX4);
                sp.setSDSBX4(r, sdsBX4);
                sp.setSOSTSX2(r, sostsX2);

                // sosmM1X2, sosmM2X2, sosbM1X2, sosbM2X2
                int sosM1X2 = 0;
                int sosBM1X2 = 0;
                int sosM2X2 = 0;
                int sosBM2X2 = 0;
                ...
                if (r == 0) {
                    ...
                    sosBM1X2 = Math.max(osbX2[0], osbX2[1]);
                    sosM2X2 = 0;
                    sosBM2X2 = 0;
                } else {
                    int rMin = 0;
                    ...
                    sosBM1X2 = sp.getSOSBX2(r) - osbX2[rMinB];
                    sosBM2X2 = sosBM1X2 - osbX2[rMinB2];
                    sp.setSOSBM1X2(r, sosBM1X2);
                    sp.setSOSBM2X2(r, sosBM2X2);
            ...
            for (ScoredPlayer sp : hmScoredPlayers.values()) {
                ...
                for (int rr = 0; rr <= r; rr++) {
                    if (sp.getParticipation(rr) != ScoredPlayer.PAIRED) {
                        ...
                        sososbX2 += sOpp.getSOSBX2(r);                       
                    }
                }
                sp.setSSSWX2(r, sososwX2);
                sp.setSSSMX2(r, sososmX2);
                sp.setSSSBX2(r, sososbX2);
           }
        }
    ...
    public void fillPairingInfo(int roundNumber) throws RemoteException {
        ...
        if (mainCrit == PlacementParameterSet.PLA_CRIT_BZS) {
            mainScoreMin = 0;
            mainScoreMax = gps.getNumberOfBZHGroups() + roundNumber;
        }
...
//                at = bt + (120 * gps.getCanByoYomiTime()) / gps.getNbMovesCanTime();
    ...
    /**
     * @return the privateTournament
     */
    public boolean isPrivateTournament() throws RemoteException{
        return privateTournament;
    }

    /**
     * @param privateTournament the privateTournament to set
     */
    public void setPrivateTournament(boolean privateTournament) throws RemoteException{
