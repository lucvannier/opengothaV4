...
public class PlayerComparator implements Comparator<Player>, Serializable{
    ...
    public final static int SBZS_RATING_ORDER = 6;
    ...
    public int compare(Player p1, Player p2){
        switch (playerOrderType){
            ...
            case RANK_ORDER :
                if (p1.getRank() < p2.getRank()) return 1; // Bad order
                if (p1.getRank() > p2.getRank()) return -1;// Good order
            ...
            case RATING_ORDER :
                if (p1.getRating() < p2.getRating()) return 1; // Bad order
                if (p1.getRating() > p2.getRating()) return -1;// Good order
                c = p1.getName().toLowerCase().compareTo(p2.getName().toLowerCase());
                if (c != 0) return c;
                else return p1.getFirstName().toLowerCase().compareTo(p2.getFirstName().toLowerCase());
            case SBZS_RATING_ORDER :
                int sbzs1 = p1.getRank() + p1.getSmmbzsCorrection();
                int sbzs2 = p2.getRank() + p2.getSmmbzsCorrection();
                if (sbzs1 < sbzs2) return 1;
                if (sbzs1 > sbzs2) return -1;
