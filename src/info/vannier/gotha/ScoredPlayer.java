...
public class ScoredPlayer extends Player implements java.io.Serializable{
    ...
    private int[] bzsX2;       // mcmahon score * 2
    ...
    private int[] bzsVirtualX2;       // mcmahon score * 2
    ...
    private int[] sosbX2;      // Sum of Opponents bzs2
    private int[] sosbM1X2;    // Sum of (n-1) Opponents bzs2
    private int[] sosbM2X2;    // Sum of (n-2) Opponents bzs2
    private int[] sdsbX4;      // Sum of Defeated Opponents bzs2 X2
    ...
    private int[] sssbX2;      // Sum of opponents sosm2 * 2
    ...
    public int getBZSX2(int rn){
        if (isValidRoundNumber(rn)) return bzsX2[rn];
        else return 0;
    }
    public void setBZSX2(int rn, int value){
        if (isValidRoundNumber(rn)) bzsX2[rn] = value;
    }
    ...
    public int getBZSVirtualX2(int rn){
        if (isValidRoundNumber(rn)) return bzsVirtualX2[rn];
        else return 0;
    }
    public void setBZSVirtualX2(int rn, int value){
        if (isValidRoundNumber(rn)) bzsVirtualX2[rn] = value;
    }
    ...
    public int getSOSBX2(int rn){
        if (isValidRoundNumber(rn)) return sosbX2[rn];
        else return 0;
    }
    public void setSOSBX2(int rn, int value){
        if (isValidRoundNumber(rn)) sosbX2[rn] = value;
    }
    public int getSOSBM1X2(int rn){
        if (isValidRoundNumber(rn)) return sosbM1X2[rn];
        else return 0;
    }
    public void setSOSBM1X2(int rn, int value){
        if (isValidRoundNumber(rn)) sosbM1X2[rn] = value;
    }
    public int getSOSBM2X2(int rn){
        if (isValidRoundNumber(rn)) return sosbM2X2[rn];
        else return 0;
    }
    public void setSOSBM2X2(int rn, int value){
        if (isValidRoundNumber(rn)) sosbM2X2[rn] = value;
    }
    public int getSDSBX4(int rn){
        if (isValidRoundNumber(rn)) return sdsbX4[rn];
        else return 0;
    }
    public void setSDSBX4(int rn, int value){
        if (isValidRoundNumber(rn)) sdsbX4[rn] = value;
    }
    ...
    public int getSSSBX2(int rn){
        if (isValidRoundNumber(rn)) return sssbX2[rn];
        else return 0;
    }
    public void setSSSBX2(int rn, int value){
        if (isValidRoundNumber(rn)) sssbX2[rn] = value;
    }
    public int getDC(){
        return dc;
    }
    ...
    public ScoredPlayer(GeneralParameterSet gps, Player player) {
        ...
        bzsVirtualX2  = new int[numberOfRounds];
        ...
        sosbX2 = new int[numberOfRounds];
        sosbM1X2 = new int[numberOfRounds];
        sosbM2X2 = new int[numberOfRounds];

        sdsbX4 = new int[numberOfRounds];
    ...
    public int getCritValue(int criterion, int rn){
        switch(criterion){
            ...
            case PlacementParameterSet.PLA_CRIT_BZS    : return  (rn >= 0) ? bzsX2[rn] : 2 * sbzs(generalParameterSet);  // Breizh score
            ...
            case PlacementParameterSet.PLA_CRIT_SOSB   : return  (rn >= 0) ? this.sosbX2[rn] : 0;	// Sum of Opponents McMahon scores
            case PlacementParameterSet.PLA_CRIT_SOSBM1 : return  (rn >= 0) ? this.sosbM1X2[rn] : 0;
            case PlacementParameterSet.PLA_CRIT_SOSBM2 : return  (rn >= 0) ? this.sosbM2X2[rn] : 0;
            case PlacementParameterSet.PLA_CRIT_SODOSB : return  (rn >= 0) ? this.getSdsbX4()[rn] : 0;	// Sum of Defeated Opponents Scores
            case PlacementParameterSet.PLA_CRIT_SOSOSB : return  (rn >= 0) ? this.sssbX2[rn] : 0;	// Sum of opponents SOS
                ...
                if (g == null){
                    ...
                    if (sp.participation[r] == ScoredPlayer.NOT_ASSIGNED) strRes = "-";
                        ...
                        if (sp.participation[r] == ScoredPlayer.ABSENT)
                            if (tps.tournamentType() == TournamentParameterSet.TYPE_MCMAHON || tps.tournamentType() == TournamentParameterSet.TYPE_BREIZH) 
                                res = gps.getGenMMSBZ2ValueAbsent();
                            else res = gps.getGenNBW2ValueAbsent();
                        else if (sp.participation[r] == ScoredPlayer.BYE)
                            if (tps.tournamentType() == TournamentParameterSet.TYPE_MCMAHON|| tps.tournamentType() == TournamentParameterSet.TYPE_BREIZH) res = gps.getGenMMSBZS2ValueBye();
