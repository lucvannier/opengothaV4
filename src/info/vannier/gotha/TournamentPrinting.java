...
public class TournamentPrinting implements Printable {
    ...
    static final int PL_BZ_BEG = PL_MM_BEG;
    static final int PL_BZ_LEN = PL_MM_LEN;
...
//            drawRightAlignedString(g, strGr, x, y);
            ...
            if (tournamentType == TournamentParameterSet.TYPE_BREIZH){
                String strBZ = "" + player.sbzs(tournament.getTournamentParameterSet().getGeneralParameterSet());
                x = usableX + usableWidth * (PL_BZ_BEG + PL_BZ_LEN) / PL_NBCAR;
                drawRightAlignedString(g, strBZ, x, y);
            }
            else{
...
//            drawRightAlignedString(g, strGr, x, y);
    ...
    private int printAPageOfTournamentParameters(Graphics g, PageFormat pf, int pi) throws RemoteException {
        ...
        if (pi == 0) {
            ...
            String strMMS = "";
            switch (gps.getGenMMSBZ2ValueAbsent()) {
                case 0:
                    strMMS = "0";
                    break;
                case 1:
            ...
            strMMS = "";
            switch (gps.getGenMMSBZS2ValueBye()) {
                case 0:
                    strMMS = "0";
                    break;
                case 1:
...
//        if (ln == 0) return NO_SUCH_PAGE;
    ...
    private void printPlayersListHeaderLine(Graphics g, PageFormat pf, int pi) {
        ...
        if (tps.tournamentType() == TournamentParameterSet.TYPE_BREIZH){
            x = usableX + usableWidth * (PL_BZ_BEG + PL_BZ_LEN) / PL_NBCAR;
            drawRightAlignedString(g, "BZ", x, y);           
        }
        else{
            x = usableX + usableWidth * (PL_MM_BEG + PL_MM_LEN) / PL_NBCAR;
