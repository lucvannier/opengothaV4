...
public class GeneralParameterSet implements java.io.Serializable{
    ...
    /**  For instance : paris2021 */
    private String shortName = "Undefined";
    /**  For instance : Toyota Tour Paris Tournament 2021 */
    ...
    // Internet
    private boolean bInternetGame = false;
    ...
    private int numberOfBZHGroups = 1;
    private int[] lowerBZHGroupLimits; // limits are defined in rank units (between +8 and -30)
    ...
    private int genMMSBZS2ValueAbsent = 1;     // 2 * Number of MMS/BZS points for a player absent of a round
    private int genMMSBZS2ValueBye = 2;        // 2 * Number of MMS/BZS points for a player not paired in a round   
    ...
    public GeneralParameterSet(GeneralParameterSet gps) {
        ...
        this.numberOfBZHGroups = gps.getNumberOfBZHGroups();
        int[] llbg = gps.getLowerBZHGroupLimits();
        int[] llbg2 = null;
        if (llbg != null){
            llbg2 = new int[llbg.length];
            System.arraycopy(llbg, 0, llbg2, 0, llbg.length);
            this.lowerBZHGroupLimits = llbg2;
        }
...
//        this.genMMZero = gps.getGenMMZero();
        ...
        this.genMMSBZS2ValueAbsent = gps.getGenMMSBZ2ValueAbsent();
        this.genMMSBZS2ValueBye = gps.getGenMMSBZS2ValueBye();
    ...
    public void initBase(String shortName, String name, String location, String director,
            java.util.Date beginDate, java.util.Date endDate, int numberOfRounds,  int numberOfCategories, int numberOfBZHGroups) {
        ...
        this.numberOfBZHGroups = numberOfBZHGroups;
    ...
    public void initForMM(){
        ...
        genMMSBZS2ValueAbsent      = 1;       
        genMMSBZS2ValueBye         = 2; 
    ...
    public void initForSwiss(){
        ...
        genMMSBZS2ValueAbsent      = 1;    // Not relevant
        genMMSBZS2ValueBye         = 2;    // Not relevant     
    ...
    public void initForSwissCat(){
        ...
        genMMSBZS2ValueAbsent      = 0;    // Not relevant
        genMMSBZS2ValueBye         = 0;    // Not relevant  
    ...
    public void initForBreizh(){
        setNumberOfCategories(1);
        setNumberOfBZHGroups(4); 
        lowerBZHGroupLimits = new int[3];
        
        lowerBZHGroupLimits[0]  = -2;
        lowerBZHGroupLimits[1]  = -7;
        lowerBZHGroupLimits[2]  = -12;
        
        genMMFloor              = -20;  // Not relevant      
        genMMBar                = 3;    // Not relevant       
        genMMZero               = -30;  // Not relevant   
...
//        if(shortName.length() < 1) shortName = "TournamentShortName";
    ...
    public int getNumberOfBZHGroups() {
        return numberOfBZHGroups;
    }

    public void setNumberOfBZHGroups(int numberOfBZHGroups) {
        this.numberOfBZHGroups = numberOfBZHGroups;

        if (numberOfBZHGroups <= 0){
            numberOfBZHGroups = 1;
        }

        // Ensure coherence between numberOfCategories and lowerCategoryLimits
        if (numberOfBZHGroups == 1){
            lowerBZHGroupLimits = null;
            return;
        }

        if (this.lowerBZHGroupLimits == null) lowerBZHGroupLimits = new int[numberOfBZHGroups - 1];

        if ( (numberOfBZHGroups - 1) < lowerBZHGroupLimits.length){
            int[] lbgl = new int[numberOfBZHGroups - 1];
            System.arraycopy(lowerBZHGroupLimits, 0, lbgl, 0, numberOfBZHGroups - 1);
            setLowerBZHGroupLimits(lbgl);
            return;
        }
        if ( (numberOfBZHGroups - 1) > lowerBZHGroupLimits.length){
            int[] lbgl = new int[numberOfBZHGroups - 1];
            System.arraycopy(lowerBZHGroupLimits, 0, lbgl, 0, lowerBZHGroupLimits.length);
            for (int c = lowerBZHGroupLimits.length; c < numberOfBZHGroups - 1; c++){
                lbgl[c] = Gotha.MIN_RANK;
            }
            setLowerCategoryLimits(lbgl);
        }
    }
    ...
    public int[] getLowerBZHGroupLimits() {
        if (lowerBZHGroupLimits == null) return null;
        int[] lcl = new int[lowerBZHGroupLimits.length];
        System.arraycopy(lowerBZHGroupLimits, 0, lcl, 0, lowerBZHGroupLimits.length);
        return lcl;
    }

    public final void setLowerBZHGroupLimits(int[] lowerBZHGroupLimits) {
        if (lowerBZHGroupLimits == null){
            this.lowerBZHGroupLimits = null;
            return;
        }
        this.lowerBZHGroupLimits = new int[lowerBZHGroupLimits.length];
        System.arraycopy(lowerBZHGroupLimits, 0, this.lowerBZHGroupLimits, 0, lowerBZHGroupLimits.length);
    }
...
//        this.genMMZero = genMMZero;
    ...
    public int getGenMMSBZ2ValueAbsent() {
        return genMMSBZS2ValueAbsent;
    }

    public void setGenMMSBZS2ValueAbsent(int genMMSBZ2ValueAbsent) {
        this.genMMSBZS2ValueAbsent = genMMSBZ2ValueAbsent;
    }

    public int getGenMMSBZS2ValueBye() {
        return genMMSBZS2ValueBye;
    }

    public void setGenMMSBZS2ValueBye(int genMMSBZ2ValueBye) {
        this.genMMSBZS2ValueBye = genMMSBZ2ValueBye;
    }
    ...
    /**
     * @return the bInternetGame
     */
    public boolean isBInternetGame() {
        return bInternetGame;
    }

    /**
     * @param val the bInternetGame to set
     */
    public void setBInternetGame(boolean val) {
        this.bInternetGame = val;
    }
